...
/// 将物理 RAM 地址映射到密集的多项式索引空间
///
/// # 功能
/// 
/// 该函数是 Jolt RAM 检查协议的核心地址转换函数,负责将稀疏的 64 位物理地址空间
/// 压缩到一个从 0 开始的连续索引空间,以便多项式能够高效表示内存状态。
///
/// # 映射规则
///
/// 1. **空地址处理**: 地址 0 被视为"未访问",返回 `None`
/// 2. **地址对齐**: 所有地址以 8 字节(64 位)为单位进行对齐处理
/// 3. **基址归零**: 通过减去内存布局中的最低地址,将地址空间平移到从 0 开始
///
/// # 数学表示
///
/// ```text
/// 物理地址空间:  [lowest_address, ∞)
/// 映射后索引:    [0, K)
/// 
/// 映射函数: index = (address - lowest_address) / 8
/// ```
///
/// # 举例说明
///
/// 假设内存布局中最低地址为 `0x1000`(4096):
///
/// | 物理地址 | 计算过程 | 映射索引 | 说明 |
/// |---------|---------|---------|------|
/// | 0x0000 | - | None | 空地址,表示无访问 |
/// | 0x1000 | (4096 - 4096) / 8 | 0 | 内存起始位置 |
/// | 0x1008 | (4104 - 4096) / 8 | 1 | 第二个 64 位字 |
/// | 0x1010 | (4112 - 4096) / 8 | 2 | 第三个 64 位字 |
/// | 0x2000 | (8192 - 4096) / 8 | 512 | 偏移 4KB 处 |
///
/// # 为什么除以 8?
///
/// Jolt VM 以 **64 位字** (8 字节) 为最小内存单元:
/// - RISC-V 是 64 位架构
/// - 寄存器宽度为 64 位
/// - 内存读写以双字 (doubleword) 为单位
///
/// 通过 `/8` 操作,将字节地址转换为字索引,使多项式变量数量减少 3 (log₂(8) = 3)。
///
/// # 参数
///
/// * `address` - 待映射的物理内存地址 (字节级)
/// * `memory_layout` - 内存布局配置,包含各区域的起始地址
///
/// # 返回值
///
/// * `Some(index)` - 映射后的密集索引,用于多项式求值
/// * `None` - 地址为 0,表示该周期未访问内存
///
/// # Panics
///
/// 当地址小于 `lowest_address` 且不为 0 时,会触发 panic。这通常表示:
/// - 内存布局配置错误
/// - Trace 数据损坏
/// - 访问了未预期的低地址区域
///
...
///
/// 该函数被以下模块调用:
/// 1. **稀疏矩阵构建** (`read_write_checking.rs`): 将 Trace 中的地址转为矩阵列索引
/// 2. **值累加证明** (`val_evaluation.rs`): 将地址映射到 `wa` 多项式的索引
/// 3. **内存状态生成** (`gen_ram_memory_states`): 将初始/最终内存映射到向量索引
#[inline(always)]
